...
export const VirtualCampusTourModal = ({ isOpen, onClose }) => {
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-950/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-3xl w-full border border-slate-200 shadow-2xl space-y-6 relative max-h-[92vh] overflow-y-auto">
        ...
        <button onClick={onClose} className="absolute top-5 right-5 p-2 rounded-full text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors z-10">
          <X className="w-5 h-5"/>
        </button>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-slate-200">
          {['physics', 'chem', 'bio', 'library', 'campus'].map((key) => (<button key={key} onClick={() => setActiveTab(key)} className={`px-4 py-2 rounded-xl text-xs font-bold capitalize transition-all whitespace-nowrap ${activeTab === key
        ...
        <div className="space-y-4">
          <div className="relative h-64 sm:h-80 rounded-2xl overflow-hidden shadow-md border border-slate-200">
            <img src={currentTour.imageUrl} alt={currentTour.title} className="w-full h-full object-cover"/>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
            {currentTour.highlights.map((h, idx) => (<div key={idx} className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold text-navy-900 flex items-center gap-2">
