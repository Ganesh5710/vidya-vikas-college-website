...
export const StreamPredictorModal = ({ isOpen, onClose, onSelectStream }) => {
    const [marks, setMarks] = useState('550');
    const [maxMarks, setMaxMarks] = useState('600');
    const [careerGoal, setCareerGoal] = useState('engineering');
    if (!isOpen)
        return null;
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-navy-950/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-xl w-full border border-slate-200 shadow-2xl space-y-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <button onClick={onClose} className="absolute top-5 right-5 p-2 rounded-full text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors">
          <X className="w-5 h-5"/>
        </button>
        ...
        <div className="space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-200">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" value={marks} onChange={(e) => setMarks(e.target.value)} placeholder="e.g. 550" className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-bold text-navy-900 focus:ring-2 focus:ring-maroon-800 outline-none"/>
            </div>

            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Out of Total Marks</label>
              <input type="number" value={maxMarks} onChange={(e) => setMaxMarks(e.target.value)} placeholder="600" className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-bold text-navy-900 focus:ring-2 focus:ring-maroon-800 outline-none"/>
            </div>
          </div>

          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">Target Career Goal</label>
            <select value={careerGoal} onChange={(e) => setCareerGoal(e.target.value)} className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs font-bold text-navy-900 focus:ring-2 focus:ring-maroon-800 outline-none bg-white">
        ...
        <div className="bg-gradient-to-br from-navy-900 to-navy-950 text-white p-6 rounded-2xl space-y-4 border border-navy-800 shadow-inner">
          ...
          {scholarshipDiscount > 0 && (<div className="p-3 rounded-xl bg-amber-500/20 border border-amber-400/40 text-amber-300 text-xs font-semibold flex items-center justify-between">
