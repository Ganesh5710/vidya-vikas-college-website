    ...
    return (<div className="w-full bg-white">
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 py-5 flex items-center justify-between gap-6">
          ...
          <div onClick={() => handleNavClick('home')} className="flex items-center shrink-0 cursor-pointer group">
            <img src={COLLEGE_DETAILS.logoUrl || '/logo.jpg'} alt={`${COLLEGE_DETAILS.name} Official Logo`} onError={(e) => { e.target.src = '/logo.jpg'; }} className="w-24 h-24 sm:w-28 sm:h-28 md:w-32 md:h-32 object-contain group-hover:scale-105 transition-transform"/>
          </div>

          {/* Center: Institutional Title & Subtitles in Classical Hierarchy */}
          <div onClick={() => handleNavClick('home')} className="text-center flex-1 cursor-pointer space-y-1">
            {/* 1. College Main Name */}
      ...
      <header className="bg-white border-y border-slate-200 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-1.5 text-xs font-bold text-slate-800">
            <button onClick={() => handleNavClick('home')} className={`p-2 rounded-xl transition-all ${activeTab === 'home' ? 'text-white bg-maroon-900 shadow-sm' : 'text-slate-700 hover:bg-slate-100 hover:text-maroon-900'}`} title="Home Page">
              <Home className="w-4 h-4"/>
            </button>

            {navItems.slice(1).map((item) => (<button key={item.id} onClick={() => handleNavClick(item.id)} className={`px-3.5 py-1.5 rounded-xl transition-all uppercase tracking-wider text-[11px] font-extrabold ${activeTab === item.id
          ...
          <div className="hidden lg:flex items-center gap-3">
            {user ? (<div className="flex items-center gap-2">
                <button onClick={() => handleNavClick('admin')} className="px-4 py-1.5 rounded-xl bg-maroon-900 text-white font-extrabold text-xs shadow flex items-center gap-1.5 hover:bg-maroon-950 transition-transform hover:scale-105">
                  <ShieldAlert className="w-3.5 h-3.5 text-amber-400"/>
                  <span>DASHBOARD ({user.role})</span>
                </button>

                <button onClick={logout} title="Sign Out Staff" className="p-1.5 rounded-xl text-slate-500 hover:text-red-600 hover:bg-slate-100">
                  <LogOut className="w-4 h-4"/>
                </button>
              </div>) : (<button onClick={() => handleNavClick('admin')} className="px-5 py-2 rounded-xl bg-maroon-900 hover:bg-maroon-950 text-white font-extrabold text-xs uppercase tracking-widest shadow transition-all hover:scale-105 flex items-center gap-2 border border-maroon-800">
          ...
          <div className="lg:hidden flex items-center justify-between w-full py-1">
            ...
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 rounded-xl text-slate-700 hover:bg-slate-100">
        ...
        {mobileMenuOpen && (<div className="lg:hidden bg-white border-t border-slate-200 px-4 py-4 space-y-2 animate-in slide-in-from-top duration-200">
            {navItems.map((item) => (<button key={item.id} onClick={() => handleNavClick(item.id)} className={`w-full text-left px-4 py-2.5 rounded-xl font-bold text-xs uppercase tracking-wider ${activeTab === item.id
            ...
            <div className="pt-2 border-t border-slate-100">
              <button onClick={() => handleNavClick('admin')} className="w-full py-2.5 rounded-xl bg-maroon-900 text-white font-extrabold text-xs uppercase tracking-wider text-center flex items-center justify-center gap-2 shadow">
