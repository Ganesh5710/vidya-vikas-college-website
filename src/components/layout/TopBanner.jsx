...
export const TopBanner = () => {
    ...
    return (<div className="bg-navy-900 text-slate-300 text-xs py-2 px-4 border-b border-navy-800">
      <div className="max-w-7xl mx-auto flex flex-wrap justify-between items-center gap-2">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-1 bg-navy-800 p-0.5 rounded border border-slate-700">
            ...
            <button onClick={() => setLanguage('en')} className={`px-1.5 py-0.5 rounded text-[11px] font-bold transition-colors ${language === 'en' ? 'bg-maroon-900 text-white' : 'text-slate-400 hover:text-white'}`}>
              EN
            </button>
            <button onClick={() => setLanguage('te')} className={`px-1.5 py-0.5 rounded text-[11px] font-bold transition-colors ${language === 'te' ? 'bg-maroon-900 text-white' : 'text-slate-400 hover:text-white'}`}>
              తెలుగు
            </button>
          </div>
        </div>
