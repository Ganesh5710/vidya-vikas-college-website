    ...
    return (<div className="space-y-12">
      ...
      <section className="relative rounded-3xl overflow-hidden shadow-2xl bg-navy-950 min-h-[460px] sm:min-h-[520px] flex items-center">
        ...
        <div className="absolute inset-0 z-0">
          {heroSlides[currentSlide]?.photoUrl ? (<img src={heroSlides[currentSlide].photoUrl} alt={heroSlides[currentSlide].title} className="w-full h-full object-cover opacity-45 transition-opacity duration-700"/>) : (<div className="w-full h-full bg-gradient-to-br from-navy-950 via-maroon-950/90 to-navy-900 flex items-center justify-center">
        ...
        <div className="absolute bottom-6 right-6 z-20 flex items-center gap-2">
          <button onClick={prevSlide} className="p-2 rounded-full bg-white/10 hover:bg-white/20 text-white backdrop-blur border border-white/20 transition-colors">
            <ChevronLeft className="w-5 h-5"/>
          </button>

          <div className="flex items-center gap-1 px-2">
            {heroSlides.map((_, idx) => (<button key={idx} onClick={() => setCurrentSlide(idx)} className={`h-2 rounded-full transition-all ${currentSlide === idx ? 'w-6 bg-amber-400' : 'w-2 bg-white/40'}`}/>))}
          </div>

          <button onClick={nextSlide} className="p-2 rounded-full bg-white/10 hover:bg-white/20 text-white backdrop-blur border border-white/20 transition-colors">
      ...
      <section className="bg-gradient-to-r from-amber-500 via-amber-400 to-amber-500 rounded-2xl p-6 sm:p-8 shadow-md flex flex-wrap items-center justify-between gap-4 border border-amber-300">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <a href="/SVVJC_Application_Form.pdf" download="SVVJC_Application_Form.pdf" target="_blank" rel="noopener noreferrer" className="px-4 py-2 rounded-lg bg-navy-900 hover:bg-navy-950 text-white font-bold text-xs shadow transition-transform hover:scale-105 flex items-center gap-1.5">
            <Download className="w-4 h-4 text-amber-400"/>
            <span>Download Application Form PDF</span>
          </a>

          <button onClick={() => setActiveTab('admissions')} className="px-5 py-2 rounded-lg bg-maroon-900 hover:bg-maroon-950 text-white font-bold text-xs uppercase tracking-wider shadow transition-transform hover:scale-105">
      ...
      <section className="bg-white rounded-3xl p-8 sm:p-10 border border-slate-200 shadow-sm grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
        <div className="space-y-4 lg:col-span-2">
          ...
          <div className="pt-2">
            <button onClick={() => setActiveTab('about')} className="inline-flex items-center gap-2 text-xs font-bold text-maroon-800 hover:text-navy-900">
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-2">
          ...
          <button onClick={() => setActiveTab('streams')} className="text-xs font-bold text-maroon-800 hover:underline flex items-center gap-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {INITIAL_STREAMS.slice(0, 3).map((stream) => (<div key={stream.id} className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow space-y-4">
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                ...
                <button onClick={() => setActiveTab('streams')} className="font-bold text-maroon-800 hover:text-navy-900">
                  Details →
                </button>
              </div>
            </div>))}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
          ...
          <div className="space-y-3">
            {notices.length > 0 ? (notices.slice(0, 4).map((notice) => (<div key={notice.id} className="p-3 rounded-lg bg-slate-50 border border-slate-200 hover:bg-slate-100 transition-colors space-y-1">
                ...
                </div>))) : (<p className="text-xs text-slate-500 italic p-4 text-center">No circulars or notices posted yet.</p>)}
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm space-y-4">
          ...
          <div className="space-y-3">
            {events.length > 0 ? (events.slice(0, 3).map((evt) => (<div key={evt.id} className="p-3 rounded-lg bg-slate-50 border border-slate-200 flex items-center gap-3">
                  <img src={evt.posterUrl || "https://images.unsplash.com/photo-1523580494863-6f3031224c94?auto=format&fit=crop&q=80&w=400"} alt={evt.title} className="w-16 h-16 rounded-lg object-cover border shrink-0"/>
                ...
                </div>))) : (<p className="text-xs text-slate-500 italic p-4 text-center">No campus events scheduled yet.</p>)}
          </div>
        </div>
