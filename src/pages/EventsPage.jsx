...
export const EventsPage = () => {
    ...
    const [activeAlbumId, setActiveAlbumId] = useState('alb-sports');
    ...
    return (<div className="space-y-10">
      ...
      <section className="space-y-4">
        ...
        {events.length === 0 ? (<EmptyState title="No Campus Events Listed Yet" description="Upcoming campus events and celebrations posted via the Staff Control Panel will instantly appear here."/>) : (<div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {events.map((evt) => (<div key={evt.id} className="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-sm space-y-3 hover:shadow-md transition-shadow flex flex-col justify-between">
                <div>
                  {evt.posterUrl && (<div onClick={() => setSelectedPhoto({ url: evt.posterUrl, title: evt.title })} className="relative w-full h-64 sm:h-72 bg-slate-100 flex items-center justify-center p-1 border-b cursor-pointer group overflow-hidden">
                      <img src={evt.posterUrl} alt="" className="absolute inset-0 w-full h-full object-cover filter blur-2xl scale-125 opacity-50 select-none pointer-events-none"/>
                      <img src={evt.posterUrl} alt={evt.title} className="relative z-10 w-full h-full object-contain rounded group-hover:scale-105 transition-transform duration-300 drop-shadow-md"/>
                  ...
                  <div className="p-5 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded ${evt.isUpcoming ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-200 text-slate-700'}`}>
