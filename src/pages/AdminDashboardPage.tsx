...
export const AdminDashboardPage: React.FC = () => {
  ...
  const [albumPhotoPreview, setAlbumPhotoPreview] = useState<string>('');
  ...
  const handleAddAlbumPhoto = (e: React.FormEvent) => {
    e.preventDefault();
            ...
            <form onSubmit={handleAddAlbumPhoto} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input 
                    type="text"
                    placeholder="e.g. Volleyball Championship Final 2025"
                    value={albumPhotoCaption}
                    onChange={(e) => setAlbumPhotoCaption(e.target.value)}
                    className="w-full px-3.5 py-2.5 rounded-lg border border-slate-300 text-xs focus:ring-2 focus:ring-maroon-800 outline-none"
                  />
                </div>
              </div>
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <label className="px-4 py-2.5 rounded-xl bg-white hover:bg-slate-100 border border-slate-300 text-slate-700 font-bold text-xs flex items-center gap-2 cursor-pointer shadow-sm">
                    ...
                    <input 
                      type="file" 
                      accept="image/*"
                      onChange={(e) => handleFileSelect(e, setAlbumPhotoPreview, 'gallery')}
                      className="hidden"
                    />
                  </label>
              ...
              <div className="pt-4 border-t border-slate-100 space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
                  {galleryPhotos.map((photo) => (
                    <div key={photo.id} className="relative group rounded-xl overflow-hidden border border-slate-200 bg-slate-900 h-28">
                      ...
                      <button 
                        ...
                        className="absolute top-1 right-1 p-1 rounded-full bg-red-600 text-white opacity-0 group-hover:opacity-100 transition-opacity shadow"
                        title="Delete Photo"
                      >
