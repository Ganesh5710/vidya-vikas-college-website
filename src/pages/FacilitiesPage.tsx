...
export const FacilitiesPage: React.FC = () => {
  ...
  const handleFacilityPhotoUpload = (
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {facilities.map((item: FacilityItem) => (
          <div 
            key={item.id} 
            className="bg-white rounded-2xl overflow-hidden border border-slate-200 shadow-sm space-y-4 hover:shadow-md transition-shadow flex flex-col justify-between"
          >
            <div>
              {/* Photo Area or Institutional Placeholder */}
              {item.photoUrl ? (
                <div 
                  onClick={() => setSelectedPhoto({ url: item.photoUrl, title: item.title })}
                  className="relative h-56 bg-slate-950 flex items-center justify-center p-1 border-b cursor-pointer group"
                >
                  <img 
                    src={item.photoUrl} 
                    alt={item.title}
                ...
                <div className="relative h-56 bg-gradient-to-br from-slate-900 via-navy-950 to-slate-900 text-slate-300 flex flex-col items-center justify-center p-6 text-center space-y-2 border-b">
                  ...
                  {isAuthenticated && (
                    <label className="mt-2 px-3 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-400 text-navy-950 text-xs font-black flex items-center gap-1.5 cursor-pointer shadow">
                      <Upload className="w-3.5 h-3.5" />
                      <span>Upload Real Photo</span>
                      <input 
                        type="file" 
                        accept="image/*"
                        onChange={(e) => handleFacilityPhotoUpload(e, item.id)}
                        className="hidden"
                      />
                    </label>
                  )}
