...
export const StudentCornerPage: React.FC = () => {
  ...
  const [activeYearTab, setActiveYearTab] = useState<'1st_year' | '2nd_year'>('1st_year');
      ...
      <section className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
        ...
        {!quizFinished ? (
          <div className="space-y-5 bg-slate-50 p-6 rounded-2xl border border-slate-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
              {quizQuestions[currentQuizIdx].options.map((opt, idx) => {
                ...
                return (
                  ...
                  >
                    <span>{opt}</span>
                    {selectedAnswer !== null && idx === quizQuestions[currentQuizIdx].correct && (
                      <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                    )}
                    {selectedAnswer !== null && idx === selectedAnswer && idx !== quizQuestions[currentQuizIdx].correct && (
            ...
            {selectedAnswer !== null && (
              <div className="p-4 rounded-xl bg-white border border-slate-200 text-xs space-y-2 animate-in fade-in">
                ...
                <div className="pt-2 flex justify-end">
                  <button
                    ...
                    className="px-5 py-2 rounded-lg bg-navy-900 text-white font-bold text-xs"
                  >
      ...
      <section className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4 border-b border-slate-100 pb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => setActiveYearTab('1st_year')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                activeYearTab === '1st_year' ? 'bg-maroon-900 text-white shadow' : 'bg-slate-100 text-slate-700'
              }`}
            >
              Class 11 (1st Year)
            </button>
            <button
              onClick={() => setActiveYearTab('2nd_year')}
              className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                activeYearTab === '2nd_year' ? 'bg-maroon-900 text-white shadow' : 'bg-slate-100 text-slate-700'
              }`}
            >
              Class 12 (2nd Year)
            </button>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-200">
              {timetableSchedule[activeYearTab].map((row, idx) => (
